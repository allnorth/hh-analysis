...
from hh_parsing.process import load_data, get_vacancies
...
with DAG(dag_id='import_vacancies_daily',
    ...
    t_load_data = PythonOperator(
        task_id='load_data',
        python_callable = partial(load_data, vacancies=get_vacancies(1)))
    ...
    t_etl_core_hub_employer = SQLExecuteQueryOperator(
        ...
        sql =   """ INSERT INTO core.hub_employer (record_source, load_date, external_id)
                    SELECT      DISTINCT
                                  'hh' as record_source
                                , now() as load_date
                                , employer_id as external_id
                    FROM        stage.vacancy AS v
                    LEFT JOIN   core.hub_vacancy AS hv ON hv.external_id = v.employer_id::varchar
                    WHERE       hv.vacancy_id IS NULL
    ...
    t_etl_core_hub_experience = SQLExecuteQueryOperator(
        ...
        sql =   """ INSERT INTO core.hub_experience (record_source, load_date, external_id)
                    SELECT      DISTINCT
                                  'hh' as record_source
                                , now() as load_date
                                , experience_id as external_id
                    FROM        stage.vacancy AS v
                    LEFT JOIN   core.hub_vacancy AS hv ON hv.external_id = v.experience_id::varchar
                    WHERE       hv.external_id IS NULL;
                """)
    ...
    t_etl_core_hub_area = SQLExecuteQueryOperator(
        ...
        sql =   """ INSERT INTO core.hub_area (record_source, load_date, external_id)
                    SELECT      DISTINCT
                                'hh' as record_source
                                , now() as load_date
                                , area_id as external_id
                    FROM        stage.vacancy AS v
                    LEFT JOIN   core.hub_vacancy AS hv ON hv.external_id = v.area_id::varchar
                    WHERE       hv.external_id IS NULL;
                """)
    ...
    t_etl_core_hub_salary = SQLExecuteQueryOperator(
        ...
        sql =   """ INSERT INTO core.hub_salary (record_source, load_date, external_id)
                    ...
                    FROM        stage.vacancy AS v
                    LEFT JOIN   core.hub_vacancy AS hv ON hv.external_id = concat(coalesce(v.salary_from, 0.0)::varchar, coalesce(v.salary_to, 0.0)::varchar, coalesce(v.salary_currency::varchar, ''), coalesce(v.is_gross, FALSE))
                    WHERE       hv.vacancy_id IS NULL;
                """)
    ...
    t_etl_core_sat_vacancy = SQLExecuteQueryOperator(
        ...
        sql =   """ MERGE INTO core.sat_vacancy AS trg
                    ...
                    SET        deleted_at = now()
                    WHERE    vacancy_id IN  (    SELECT hv.vacancy_id
                                                FROM core.hub_vacancy hv
                                                LEFT JOIN stage.vacancy v on hv.external_id = v.id::varchar
                                                WHERE v.id IS NULL
                                            );
                """)
    ...
    t_etl_core_sat_employer = SQLExecuteQueryOperator(
        ...
        sql =   """ MERGE INTO core.sat_employer AS trg
                    USING    (    SELECT    DISTINCT
                                ...
                                JOIN    stage.vacancy AS v ON v.id::varchar = he.external_id
                            ) AS src
                    ...
                    UPDATE  core.sat_employer
                    SET     deleted_at = now()
                    WHERE   employer_id IN  (   SELECT he.employer_id
                                                FROM core.hub_employer he
                                                LEFT JOIN stage.vacancy v on he.external_id = v.employer_id::varchar
    ...
    t_etl_core_sat_experience = SQLExecuteQueryOperator(
        ...
        sql =   """ MERGE INTO core.sat_experience AS trg
                    ...
                    UPDATE  core.sat_experience
                    SET     deleted_at = now()
                    WHERE   experience_id IN    (   SELECT he.experience_id
                                                    FROM core.hub_experience he
                                                    LEFT JOIN stage.vacancy v on he.external_id = v.experience_id::varchar
    ...
    t_etl_core_sat_area = SQLExecuteQueryOperator(
        ...
        sql =   """ MERGE INTO core.sat_area AS trg
                    ...
                    UPDATE  core.sat_area
                    SET     deleted_at = now()
                    WHERE   area_id IN  (   SELECT ha.area_id
                                            FROM core.hub_area ha
                                            LEFT JOIN stage.vacancy v on ha.external_id = v.area_id::varchar
